layui.define(["jquery","layer"], function (exports) {
	...
	theme.colorSet = function(color) {
		...
		let style = '';
		style += '.light-theme .gsadmin-nav-tree .layui-this a:hover,.light-theme .gsadmin-nav-tree .layui-this,.light-theme .gsadmin-nav-tree .layui-this a,.gsadmin-nav-tree .layui-this a,.gsadmin-nav-tree .layui-this{background-color: ' +color + '!important;}';
		style += '.gsadmin-admin .layui-logo .title{color:' + color + '!important;}';
		style += '.gsadmin-frame-title .dot,.gsadmin-tab .layui-this .gsadmin-tab-active{background-color: ' + color +'!important;}';
		style += '.bottom-nav li a:hover{background-color:' + color + '!important;}';
		style += '.gsadmin-admin .layui-header .layui-nav .layui-nav-bar{background-color: ' + color + '!important;}'
		...
		style += '.gsadminone-color .color-content li.layui-this:after, .gsadminone-color .color-content li:hover:after {border: ' +color + ' 3px solid!important;}';
		style += '.layui-nav .layui-nav-child dd.layui-this a, .layui-nav-child dd.layui-this{background-color:' + color + '!important}';	
		style += '.gsadmin-social-entrance {background-color:' + color + '!important}';
		style += '.gsadmin-admin .pe-collaspe {background-color:' + color + '!important}';
		style += '.layui-fixbar li {background-color:' + color + '!important}';
		if(this.autoHead){
			style += '.gsadmin-admin .layui-header{background-color:' + color + '!important;}.gsadmin-admin .layui-header .layui-nav .layui-nav-item>a{color:white!important;}';
		}
		style += '.gsadmin-btn-primary {background-color:' + color + '!important}';
		...
		style += '.gsadmin-text{color:' + color + '!important}';
		style += '.gsadmin-collasped-pe{background-color:'+color+'!important}'
		...
		style += '.gsadmin-tab-menu .item:hover{background-color:'+color+'!important}'
		style += '.layui-form-danger:focus {border-color:#FF5722 !important}'
		style += '.gsadmin-admin .user a:hover{color:'+color+'!important}'
		style += '.gsadmin-admin .user .layui-this a:hover{color:white!important}'
		style += '.gsadmin-notice .layui-this{color:'+color+'!important}'
		var colorPane = $("#gsadmin-admin-color");
		if(colorPane.length>0){
			colorPane.html(style);
		}else{
			$("head").append("<style id='gsadmin-admin-color'>"+style+"</style>")
		}
	}
