layui.define(['jquery', 'element'], function(exports) {
	...
	gsadminTab.prototype.render = function(opt) {
		//默认配置值
		var option = {
			elem: opt.elem,
			data: opt.data,
		...
		if (option.session) {
			// 替换 opt.data 数据
			if (sessionStorage.getItem(option.elem + "-gsadmin-tab-data") != null) {
				tabData = JSON.parse(sessionStorage.getItem(option.elem + "-gsadmin-tab-data"));
				option.data = JSON.parse(sessionStorage.getItem(option.elem + "-gsadmin-tab-data"));
				tabDataCurrent = sessionStorage.getItem(option.elem + "-gsadmin-tab-data-current");
		...
		option.success(sessionStorage.getItem(option.elem + "-gsadmin-tab-data-current"));
		...
		return new gsadminTab(option);
	}

	gsadminTab.prototype.click = function(callback) {
		var elem = this.option.elem;
		var option = this.option;
		element.on('tab(' + this.option.elem + ')', function(data) {
			var id = $("#" + elem + " .layui-tab-title .layui-this").attr("lay-id");
			sessionStorage.setItem(option.elem + "-gsadmin-tab-data-current", id);
			callback(id);
		});
	}

	gsadminTab.prototype.positionTab = function() {
	...
	gsadminTab.prototype.clear = function() {
		sessionStorage.removeItem(this.option.elem + "-gsadmin-tab-data");
		sessionStorage.removeItem(this.option.elem + "-gsadmin-tab-data-current");
	}

	gsadminTab.prototype.addTab = function(opt) {
		var title = '';
		if (opt.close) {
			title += '<span class="gsadmin-tab-active"></span><span class="able-close">' + opt.title +
				'</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>';
		} else {
			title += '<span class="gsadmin-tab-active"></span><span class="disable-close">' + opt.title +
				'</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>';
		}
		...
		tabData.push(opt);
		sessionStorage.setItem(this.option.elem + "-gsadmin-tab-data", JSON.stringify(tabData));
		sessionStorage.setItem(this.option.elem + "-gsadmin-tab-data-current", opt.id);
	...
	gsadminTab.prototype.delTabByElem = function(elem, id, callback) {
	...
	gsadminTab.prototype.delCurrentTabByElem = function(elem, callback) {
	...
	gsadminTab.prototype.addTabOnlyByElem = function(elem, opt, time) {
		var title = '';
		if (opt.close) {
			title += '<span class="gsadmin-tab-active"></span><span class="able-close">' + opt.title +
				'</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>'
		...
		if ($(".layui-tab[lay-filter='" + elem + "'] .layui-tab-title li[lay-id]").length <= 0) {
			...
			if (time != false && time != 0) {
				...
				var load = '<div id="gsadmin-tab-loading' + index + '" class="gsadmin-tab-loading">' +
					'<div class="ball-loader">' +
					'<span></span><span></span><span></span><span></span>' +
					'</div>' +
					'</div>'
				$("#" + elem).find(".gsadmin-tab").append(load);
				var gsadminLoad = $("#" + elem).find("#gsadmin-tab-loading" + index);
				gsadminLoad.css({
					display: "block"
				});
				setTimeout(function() {
					gsadminLoad.fadeOut(500);
				}, time);
				index++;
			}
			element.tabAdd(elem, {
			...
			tabData.push(opt);
			sessionStorage.setItem(elem + "-gsadmin-tab-data", JSON.stringify(tabData));
		} else {
			var isData = false;
		...
		sessionStorage.setItem(elem + "-gsadmin-tab-data-current", opt.id);
		element.tabChange(elem, opt.id);
	}

	/** 添 加 唯 一 选 项 卡 */
	gsadminTab.prototype.addTabOnly = function(opt, time) {
		var title = '';
		if (opt.close) {
			title += '<span class="gsadmin-tab-active"></span><span class="able-close">' + opt.title +
				'</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>';
		...
		if ($(".layui-tab[lay-filter='" + this.option.elem + "'] .layui-tab-title li[lay-id]").length <= 0) {
			if (time != false && time != 0) {
				var load = '<div id="gsadmin-tab-loading' + index + '" class="gsadmin-tab-loading">' +
					...
					'</div>' +
					'</div>';
				$("#" + this.option.elem).find(".gsadmin-tab").append(load);
				var gsadminLoad = $("#" + this.option.elem).find("#gsadmin-tab-loading" + index);
				gsadminLoad.css({
					display: "block"
				});
				setTimeout(function() {
	...
	// 刷 新 指 定 的 选 项 卡
	gsadminTab.prototype.refresh = function(time) {
		// 刷 新 指 定 的 选 项 卡
		if (time != false && time != 0) {
			var load = '<div id="gsadmin-tab-loading' + index + '" class="gsadmin-tab-loading">' +
				'<div class="ball-loader">' +
			...
			index++;
			setTimeout(function() {
				gsadminLoad.fadeOut(500, function() {
					gsadminLoad.remove();
				});
			}, time);
	...
	function tabDelete(elem, id, callback, option) {
		...
		if (!removeTab.hasClass("layui-this")) {
			...
			tabData = JSON.parse(sessionStorage.getItem(elem + "-gsadmin-tab-data"));
			tabDataCurrent = sessionStorage.getItem(elem + "-gsadmin-tab-data-current");
		...
		sessionStorage.setItem(elem + "-gsadmin-tab-data", JSON.stringify(tabData));
		sessionStorage.setItem(elem + "-gsadmin-tab-data-current", currId);
	...
	function createTab(option) {
		...
		var tab = '<div class="gsadmin-tab ' + types + type + ' layui-tab" lay-filter="' + option.elem +
			'" lay-allowClose="true">';
		...
		$.each(option.data, function(i, item) {
			...
			if (option.index == index) {
				TitleItem += '<li lay-id="' + item.id + '" class="layui-this"><span class="gsadmin-tab-active"></span>';
			} else {
				TitleItem += '<li lay-id="' + item.id + '" ><span class="gsadmin-tab-active"></span>';
			}
	...
	exports(MOD_NAME, new gsadminTab());
})
