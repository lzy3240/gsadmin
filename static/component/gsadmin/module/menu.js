layui.define(['table', 'jquery', 'element'], function(exports) {
	...
	var gsadminMenu = function(opt) {
		this.option = opt;
	};

	gsadminMenu.prototype.render = function(opt) {
		...
		return new gsadminMenu(opt);
	}

	gsadminMenu.prototype.click = function(clickEvent) {
		var _this = this;
		$("body").on("click", "#" + _this.option.elem + " .site-demo-active", function() {
			...
			if ($("#" + _this.option.elem).is(".gsadmin-nav-mini")) {
	...
	gsadminMenu.prototype.skin = function(skin) {
		var menu = $(".gsadmin-nav-tree[lay-filter='" + this.option.elem + "']").parent();
		menu.removeClass("dark-theme");
		menu.removeClass("light-theme");
		menu.addClass(skin);
	}

	gsadminMenu.prototype.selectItem = function(gsadminId) {
		if (this.option.control != false) {
			$("#" + this.option.elem + " a[menu-id='" + gsadminId + "']").parents(".layui-side-scroll ").find("ul").css({
				display: "none"
			});
			$("#" + this.option.elem + " a[menu-id='" + gsadminId + "']").parents(".layui-side-scroll ").find(".layui-this").removeClass(
				"layui-this");
			$("#" + this.option.elem + " a[menu-id='" + gsadminId + "']").parents("ul").css({
				display: "block"
			});
			var controlId = $("#" + this.option.elem + " a[menu-id='" + gsadminId + "']").parents("ul").attr("gsadmin-id");
			if (controlId != undefined) {
				$("#" + this.option.control).find(".layui-this").removeClass("layui-this");
				$("#" + this.option.control).find("[gsadmin-id='" + controlId + "']").addClass("layui-this");
			}
		}
		if (this.option.accordion == true) {
			$("#" + this.option.elem + " a[menu-id='" + gsadminId + "']").parents(".gsadmin-nav-tree").find(".layui-nav-itemed").removeClass(
				"layui-nav-itemed");
		}
		$("#" + this.option.elem + " a[menu-id='" + gsadminId + "']").parents(".gsadmin-nav-tree").find(".layui-this").removeClass(
			"layui-this");
		if (!$("#" + this.option.elem).is(".gsadmin-nav-mini")) {
			$("#" + this.option.elem + " a[menu-id='" + gsadminId + "']").parents(".layui-nav-item").addClass("layui-nav-itemed");
			$("#" + this.option.elem + " a[menu-id='" + gsadminId + "']").parents("dd").addClass("layui-nav-itemed");
		}
		$("#" + this.option.elem + " a[menu-id='" + gsadminId + "']").parent().addClass("layui-this");
	}

	var activeMenus;
	gsadminMenu.prototype.collaspe = function(time) {
		...
		if ($("#" + this.option.elem).is(".gsadmin-nav-mini")) {
			...
			$("#" + this.option.elem).removeClass("gsadmin-nav-mini");
			...
			$("#" + this.option.elem).addClass("gsadmin-nav-mini");
			$("#" + this.option.elem).animate({
				width: "60px"
			}, 400);
	...
	function createMenu(option) {
		var menuHtml = '<div style="height:100%!important;" class="gsadmin-side-scroll layui-side-scroll ' + option.theme + '"><ul lay-filter="' + option.elem +
			'" class="layui-nav arrow   gsadmin-menu layui-nav-tree gsadmin-nav-tree">'
	...
	function createMenuAndControl(option) {
		var control = '<ul class="layui-nav  gsadmin-nav-control pc layui-hide-xs">';
		var controlPe = '<ul class="layui-nav gsadmin-nav-control layui-hide-sm">';
		// 声 明 头 部
		...
		$.each(option.data, function(i, item) {
			...
			if (i === option.defaultMenu) {
				controlItem = '<li gsadmin-href="' + item.href + '" gsadmin-title="' + item.title + '" gsadmin-id="' + item.id +
					'" class="layui-this layui-nav-item"><a href="#">' + item.title + '</a></li>';
				menuItem = '<ul  gsadmin-id="' + item.id + '" lay-filter="' + option.elem +
					'" class="layui-nav arrow layui-nav-tree gsadmin-nav-tree">';
				// 兼容移动端
				controlPe += '<li class="layui-nav-item"><a class="pe-title" href="javascript:;" >' + item.title + '</a>';
				controlItemPe += '<dd  gsadmin-href="' + item.href + '" gsadmin-title="' + item.title + '" gsadmin-id="' + item.id +
					'"><a href="javascript:void(0);">' + item.title + '</a></dd>';
			} else {
				controlItem = '<li  gsadmin-href="' + item.href + '" gsadmin-title="' + item.title + '" gsadmin-id="' + item.id +
					'" class="layui-nav-item"><a href="#">' + item.title + '</a></li>';
				menuItem = '<ul style="display:none" gsadmin-id="' + item.id + '" lay-filter="' + option.elem +
					'" class="layui-nav arrow layui-nav-tree gsadmin-nav-tree">';
		...
		$("#" + option.control + " .gsadmin-nav-control").on("click", "[gsadmin-id]", function() {
			$("#" + option.elem).find(".gsadmin-nav-tree").css({
				display: 'none'
			});
			$("#" + option.elem).find(".gsadmin-nav-tree[gsadmin-id='" + $(this).attr("gsadmin-id") + "']").css({
				display: 'block'
			});
			$("#" + option.control).find(".pe-title").html($(this).attr("gsadmin-title"));
			$("#" + option.control).find("")
			option.change($(this).attr("gsadmin-id"), $(this).attr("gsadmin-title"), $(this).attr("gsadmin-href"))
		})
	}
	...
	function downShow(option) {
		$("body #" + option.elem).on("click", "a[menu-type='0']", function() {
			if (!$("#" + option.elem).is(".gsadmin-nav-mini")) {
	...
	function isHoverMenu(b, option) {
		if (b) {
			$("#" + option.elem + ".gsadmin-nav-mini .layui-nav-item,#" + option.elem + ".gsadmin-nav-mini dd").hover(function(e) {
				e.stopPropagation();
				var _this = $(this);
	...
	exports(MOD_NAME, new gsadminMenu());
})
