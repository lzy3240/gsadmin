...
// 将html转成markdown
func (cron Cron) Html2Markdown(htmlstr string) (md string) {
...
func handleImg(doc *goquery.Document) *goquery.Document {
	doc.Find("img").Each(func(i int, selection *goquery.Selection) {
		if src, ok := selection.Attr("src"); ok {
			...
			fpath, _ := saveImg(src)
			md := fmt.Sprintf(`![%v](/%v)`, alt, strings.ReplaceAll(fpath, "\\", "/"))
			selection.BeforeHtml(md)
			selection.Remove()
		}
	})
...
func getInnerHtml(selection *goquery.Selection) (html string) {
	...
	if err != nil {
		//logs.Error("解析页面失败 -->", err)
	}
	return
}

func saveImg(img_url string) (fpath string, err error) {
	// 下载图片
	...
	// 保存图片
	ext := filepath.Ext(img_url)
	if ext == "" {
		ext = ".jpg"
	}

	fileName := assertion.AnyToString(time.Now().UnixMilli()) + ext
	savePath := filepath.Join(config.Instance().App.FileSavePath, time.Now().Format("20060102"), fileName)
	...
	showPath := filepath.Join(config.Instance().App.FileViewPath, time.Now().Format("20060102"), fileName)
	return showPath, nil
}
